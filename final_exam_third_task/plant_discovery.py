...
while command != "Exhibition":
    ...
    if explode[0] == "Rate":
        ...
        if plants in plant_dict:
            plant_dict[plants].pop(0)
            plant_dict[plants].insert(0, new_rarity)
        else:
            print("error")
        ...
        plants = explode[1]
        if plants in plant_dict:
            rarity_to_add_back = plant_dict[plants][0]
            plant_dict[plants].clear()
            plant_dict[plants].append(rarity_to_add_back)
            #plant_dict[plants].insert(1, 0)
        else:
            print("error")
